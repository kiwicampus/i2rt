...
# -- get_gpio_backend: platform detection --------------------------------------


@pytest.fixture
def no_singleton(monkeypatch: pytest.MonkeyPatch) -> None:
    """Fresh backend singleton so each test resolves the backend on its own."""
    monkeypatch.setattr(usb_gpio_driver, "_BACKEND_SINGLETON", None)


@pytest.mark.parametrize(
    ("machine", "model", "expected"),
    [
        ("x86_64", "", False),
        ("x86_64", "Raspberry Pi 4 Model B Rev 1.4", False),
        ("aarch64", "Raspberry Pi 4 Model B Rev 1.4", True),
        ("aarch64", "NVIDIA Jetson Orin Nano Developer Kit", False),
        ("aarch64", "", True),
        ("armv7l", "Raspberry Pi 3 Model B Plus Rev 1.3", True),
    ],
)
def test_is_raspberry_pi_uses_device_tree_model(
    monkeypatch: pytest.MonkeyPatch, machine: str, model: str, expected: bool
) -> None:
    monkeypatch.setattr(usb_gpio_driver.platform, "machine", lambda: machine)
    monkeypatch.setattr(usb_gpio_driver, "_device_tree_model", lambda: model)
    assert usb_gpio_driver.is_raspberry_pi() is expected


def test_device_tree_model_reads_first_available_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    model_file = tmp_path / "model"
    model_file.write_bytes(b"NVIDIA Jetson Orin Nano Developer Kit\x00")
    monkeypatch.setattr(usb_gpio_driver, "_DEVICE_TREE_MODEL_PATHS", (str(tmp_path / "missing"), str(model_file)))
    assert usb_gpio_driver._device_tree_model() == "NVIDIA Jetson Orin Nano Developer Kit"


def test_device_tree_model_empty_when_unreadable(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    monkeypatch.setattr(usb_gpio_driver, "_DEVICE_TREE_MODEL_PATHS", (str(tmp_path / "missing"),))
    assert usb_gpio_driver._device_tree_model() == ""


def test_get_gpio_backend_non_pi_arm_uses_usb_backend(monkeypatch: pytest.MonkeyPatch, no_singleton: None) -> None:
    monkeypatch.setattr(usb_gpio_driver, "is_raspberry_pi", lambda: False)
    backend = usb_gpio_driver.get_gpio_backend(pin_map=PIN_MAP)
    assert isinstance(backend, UsbGpioBackend)


def test_get_gpio_backend_falls_back_when_rpi_missing(
    monkeypatch: pytest.MonkeyPatch, no_singleton: None, caplog: pytest.LogCaptureFixture
) -> None:
    monkeypatch.setattr(usb_gpio_driver, "is_raspberry_pi", lambda: True)
    monkeypatch.setitem(__import__("sys").modules, "RPi", None)  # makes `from RPi import GPIO` raise
    with caplog.at_level(logging.WARNING, logger=usb_gpio_driver.__name__):
        backend = usb_gpio_driver.get_gpio_backend(pin_map=PIN_MAP)
    assert isinstance(backend, UsbGpioBackend)
    assert any("RPi.GPIO is not installed" in r.getMessage() for r in caplog.records)
