...
# Board model string exposed by the kernel; a Jetson or other ARM board reports its own name here.
_DEVICE_TREE_MODEL_PATHS = ("/proc/device-tree/model", "/sys/firmware/devicetree/base/model")


def _device_tree_model() -> str:
    """Board model string from the device tree, or an empty string when it is unavailable."""
...
def is_raspberry_pi() -> bool:
    """True on a Raspberry Pi, where GPIO is native and no USB-GPIO converter is used.
    Requires ARM; the device tree model then rules out boards such as Jetson, and decides by ARM alone if unreadable.
    """
    if platform.machine().lower() not in _RASPBERRY_PI_MACHINES:
        return False
    model = _device_tree_model()
    return "raspberry pi" in model.lower() if model else True
...
def get_gpio_backend(port: Optional[str] = None, pin_map: Optional[dict[int, int]] = None) -> Any:
    ...
    if is_raspberry_pi():
        try:
            ...
            # ARM board without RPi.GPIO: fall back to the converter instead of failing at import.
            logger.warning("RPi.GPIO is not installed on this ARM board, using the USB-GPIO backend")
